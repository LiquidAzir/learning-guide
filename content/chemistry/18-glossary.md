...
**Atomic mass.** The average mass of an element's atoms, weighted over its natural isotopes, in atomic mass units. Numerically equal to the mass in grams of one mole of those atoms.

**Atomic number.** The number of protons in a nucleus. It defines the element and orders the periodic table.

**Avogadro's number.** $6.022 \times 10^{23}$, the number of things in a mole. Fixed exactly by definition since 2019.

**Balanced equation.** A chemical equation with equal numbers of each kind of atom on both sides, as conservation of mass requires. Its coefficients give the recipe in moles.
...
**Ion.** An atom or molecule with unequal numbers of protons and electrons, so it carries a charge. A positive one is a **cation**, a negative one an **anion**.

**Isomers.** Compounds with the same molecular formula but different structures, and therefore different properties.

**Isotopes.** Atoms of one element with different numbers of neutrons. Same chemistry, different mass, sometimes radioactive.

**Kilojoule per mole (kJ/mol).** The standard energy unit of chemistry. A chemical bond is worth 200 to 500; one electron-volt per molecule is 96.5.
...
**Aufbau principle.** Electrons fill the lowest-energy orbitals first, with **Hund's rule** spreading them one per equal-energy orbital before pairing.

**Conservation of mass.** Total mass is unchanged in a chemical reaction. Lavoisier, 1789.

**Effective nuclear charge.** The net pull an outer electron feels after inner electrons screen the nucleus. It explains every periodic trend.

**Electron configuration.** The list of which orbitals an atom's electrons occupy, such as 1s² 2s² 2p⁶ 3s¹ for sodium.

**Electronegativity.** An atom's pull on shared electrons in a bond. Highest at fluorine, lowest at cesium. The difference between two atoms decides how polar their bond is.
...
**Colligative properties.** Effects of dissolved particles that depend only on how many there are: freezing-point lowering, boiling-point raising, **osmosis** (solvent flow through a membrane toward higher solute concentration).

**Covalent bond.** A shared pair of electrons. Double and triple bonds share two or three pairs and are shorter and stronger.

**Critical point.** The temperature and pressure beyond which liquid and gas become one **supercritical fluid**.

**Dispersion (London) forces.** Weak attractions between all molecules, caused by fleeting shifts of electron density. They grow with molecular size.

**Hydrogen bond.** The strong attraction between a hydrogen already bonded to N, O, or F and a lone pair on a neighbor. About 20 kJ/mol. Responsible for water's oddities and DNA's pairing.

**Ideal gas law.** $PV = nRT$: a gas's pressure and volume are set by how many molecules and how hot, regardless of which gas.

**Intermolecular forces.** Attractions between molecules, ten to a hundred times weaker than bonds. They set melting and boiling points.

**Ionic bond.** The electrostatic attraction between oppositely charged ions in a lattice. **Lattice energy** is the energy released in assembling that lattice.

**Lewis structure and the octet rule.** A dot diagram of all outer electrons; the tendency of second-row atoms to end up surrounded by eight.

**Molecular orbital theory.** Electrons occupy orbitals spread over the whole molecule, bonding (lower energy) or antibonding (higher). Correctly predicts oxygen's magnetism.

**Phase diagram.** A map of which state a substance takes at each temperature and pressure, with a **triple point** where all three coexist.
...
**VSEPR.** Valence shell electron pair repulsion: electron pairs around an atom get as far apart as possible, which sets molecular shapes. Water's two lone pairs make it bent.

## Change (chapters 8 to 11)

**Acid and base.** A proton donor and a proton acceptor (Brønsted–Lowry); or an electron-pair acceptor and donor (Lewis). Every acid has a **conjugate base**.

**Activation energy ($E_a$).** The minimum collision energy needed for a reaction. It sets the rate through the **Arrhenius equation**, $k = A e^{-E_a/RT}$.

**Buffer.** A mixture of a weak acid and its conjugate base that resists pH change. Blood and seawater are buffered.

**Catalyst.** A substance that speeds a reaction by offering a lower-energy path and is not consumed. It changes the rate, never the equilibrium position. **Homogeneous** catalysts are dissolved with the reactants; **heterogeneous** ones are solids on whose surface the reaction happens. Real catalysts eventually die by **poisoning** (impurities blocking the active surface) or **sintering** (fine particles merging); the **turnover number** counts how many molecules one site converts before that happens.

**Directed evolution.** Breeding an enzyme rather than designing it: mutate the gene at random, screen thousands of the resulting proteins for the reaction you want, keep the best, repeat. Frances Arnold, Nobel Prize 2018.

**Dynamic equilibrium.** The state where forward and reverse reactions run at equal rates, so nothing appears to change. Described by the **equilibrium constant** $K$.

**Electrochemistry.** Redox with the electron transfer routed through a wire. A **galvanic cell** (battery) releases energy; **electrolysis** consumes it to drive a reaction backward. The **Nernst equation** gives the voltage's dependence on concentration.

**Endothermic and exothermic.** Absorbing heat ($\Delta H$ positive) and releasing it ($\Delta H$ negative).

**Enthalpy ($\Delta H$).** The heat of a reaction at constant pressure. **Hess's law**: it is the same by any path, so known reactions can be added to get unknown ones.

**Entropy ($\Delta S$).** A measure of how many ways a state can be arranged. Increases when solids dissolve, liquids boil, or one molecule becomes several.

**Enzyme.** A protein catalyst. Fits its **substrate** in an **active site**; can accelerate reactions by a factor of $10^{17}$.

**Gibbs free energy ($\Delta G = \Delta H - T\Delta S$).** The quantity that decides whether a reaction proceeds: negative means yes. Linked to how far it goes by $\Delta G^\circ = -RT\ln K$.

**Haber–Bosch process.** Making ammonia from atmospheric nitrogen and hydrogen over an iron catalyst at 400 to 500 °C and 150 to 300 atmospheres. It supplies most of the world's fertilizer and roughly half the nitrogen atoms in your body.

**Half-life.** The time for half of a substance to be consumed. Fixed, regardless of starting amount, for a first-order process.

**Hydronium ion (H₃O⁺).** What a "hydrogen ion in water" actually is: a proton attached to a water molecule. Its concentration sets the pH. A substance that can act as either acid or base, such as water, is **amphoteric**.

**Le Chatelier's principle.** Disturb an equilibrium and it shifts to partly undo the disturbance. The basis of industrial reactor design.

**Mechanism.** The actual sequence of elementary steps by which a reaction happens, passing through short-lived **intermediates**. The slowest step is **rate-determining**.

**Oxidation and reduction (redox).** Loss and gain of electrons; they always happen together. **Oxidation state** tracks them. An **oxidizing agent** takes electrons, a **reducing agent** donates them.

**p$K_a$.** $-\log K_a$; a measure of acid strength. Lower means stronger. Protons move from lower p$K_a$ to higher.

**Rate law.** How a reaction's speed depends on concentrations, measured not deduced. First order means rate $= k[\mathrm{A}]$.

**Solubility product ($K_{sp}$).** The equilibrium constant for a sparingly soluble salt dissolving.

**Strong and weak acids.** Those that give up essentially all their protons, and those that reach an equilibrium instead.

**Titration.** Adding a measured reagent until the reaction is exactly complete, to determine how much was present. The moment of completion is the **equivalence point**, marked by an **indicator**, a dye that changes color near the right pH.

**Transition state.** The fleeting highest-energy arrangement between reactants and products, lasting about $10^{-13}$ seconds. Its height above the reactants is the activation energy.

## The elements at work (chapters 12 to 15)
...
**Doping.** Adding trace impurities to a semiconductor to control its conductivity, one atom in a million.

**Functional group.** A small recurring arrangement of atoms (–OH, –COOH, C=O) that gives a molecule its characteristic reactivity, whatever the skeleton.
...
**Metal-organic framework (MOF).** A crystalline scaffold of metal ions and organic linkers, mostly empty space, with enormous internal surface area. Tunable for gas capture and water harvesting.
...
**Transcription and translation.** Copying DNA into RNA; reading RNA to build a protein at the **ribosome**.

**X-ray crystallography.** Determining atomic positions from how a crystal diffracts X-rays. The source of most known molecular structures.
...
**Direct air capture.** Removing carbon dioxide from ordinary air, where it is well under a twentieth of a percent of the gas present, using a material that binds it selectively and releases it cheaply.

**Green chemistry.** Designing processes to prevent waste, use catalysts and renewable feedstocks, and make products that degrade.

**PFAS.** Per- and polyfluoroalkyl substances, "forever chemicals," persistent because the carbon-fluorine bond is so strong.

**Self-driving laboratory.** A robotic system that designs, runs, and interprets its own experiments in a loop.

**Skeletal editing.** Changing a molecule's core framework by deleting, inserting, or swapping a single atom, rather than rebuilding it.
